...
export class Init1769599359919 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`messages\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`content\` text NULL, \`media_url\` text NULL, \`deleted_for_user_ids\` text NULL, \`is_recalled\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`conversation_id\` bigint NULL, \`sender_id\` bigint NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`conversation_members\` (\`conversation_id\` bigint NOT NULL, \`user_id\` bigint NOT NULL, \`nickname\` varchar(255) NULL, \`muted_until\` datetime NULL, \`muted_forever\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`conversation_id\`, \`user_id\`)) ENGINE=InnoDB`);
